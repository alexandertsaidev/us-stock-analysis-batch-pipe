    ...
    try:
        with get_duckdb_conn() as conn:
            ...
            arrow_table = get_pa_table(
                conn = conn,
                bucket = MINIO_BUCKET,
                object_name = f"stock/history/prices/gold/conclusion/exit_1_conclusion.parquet"
            )

            conn.register("exit_1_conclusion", arrow_table)

            df = conn.execute(f"""
                SELECT * FROM "exit_1_conclusion"
                ORDER BY "Date_D" DESC, "ticker" ASC
            """).df().fillna("None")

        time.sleep(1)
        rewrite_gsheet(df, "exit_1_conclusion", "US")

        slack_batch_pipe_notify(f"🔗 6. 已上傳 exit_1_conclusion 至 Google Sheet\n  • {os.environ['GSHEET_EXIT_1_URL']}")
